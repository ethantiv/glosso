...
- **Popup** (`Sources/Popup/`) — `TranslationPopupController` shows a borderless non-activating `FloatingPanel` that never becomes key (won't steal focus). Esc dismisses it (via the CGEventTap below) and the title-bar close button closes it; a click *outside* the window closes only an open alternatives dropdown (issue #30), never the panel — that outside-click-to-dismiss-the-panel was deliberately dropped in `5f4a549` once the window became movable with a close button. **The controller owns the window size** — the only stable arrangement found (issue #43's resize work): `AutoSizingHostingView` (`NSHostingView`, `sizingOptions = [.intrinsicContentSize]`) only *reports* the content's ideal size via `invalidateIntrinsicContentSize`/`layout` hooks, and `applyContentSize()` (coalesced to the next runloop turn) sets the frame with the top-left pinned so the panel grows downward, not up over the cursor. Do **not** switch to `.preferredContentSize` window-driving: the hosting view then resizes the window from inside window layout (`updateAnimatedWindowSize`) and live grip-resizing recurses until the stack overflows; competing `setFrame` calls get reverted synchronously, and pinning the SwiftUI root frame to a window size recurses layout the same way. The text panes cap height at 400pt and scroll; the bottom-right grip (`ResizeGripArea`, an `NSView` — a SwiftUI `DragGesture` never receives the drag because window-background dragging claims it) stretches pane widths and that cap via `PopupModel.sizeDelta`, and the window follows the grown content through the same single path. Shadows are drawn in SwiftUI, not by AppKit (`FloatingPanel.hasShadow` is off): the system window shadow rings the dropdown's protruding part with a hard contour, so the panel and the dropdown each carry their own `.shadow()`. The window reserves a transparent `PopupView.shadowMargin` around the visible panel for the blur, and `present()` shifts the window top-left up-left by that margin to keep the panel under the cursor — any positioning change must account for it. In the `.done` phase the result becomes a `FlowLayout` of individually clickable words (`WordFlow.swift`: `Tokenizer`, `FlowLayout`, `AlternativesDropdown`); clicking one opens an alternatives dropdown rendered as an **in-window SwiftUI overlay** (`overlayPreferenceValue` anchored to the word) — deliberately **not** an `NSPopover`/`Menu`, which would force the panel to become key and break the non-activating focus model. Esc is layered: it closes an open dropdown first, then dismisses the panel. This trades away drag-to-select on the result (the header Copy button still copies the whole translation).
