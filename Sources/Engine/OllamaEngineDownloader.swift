...
final class OllamaEngineDownloader: Sendable {
    ...
    typealias Download = @Sendable (URL, URL, @escaping @Sendable (Double) -> Void) async throws -> Void
    typealias Run = @Sendable (String, [String]) throws -> Void
    private let directory: URL
    private let temporaryDirectory: URL
    private let downloadFile: Download
    private let runCommand: Run
    private let notify: @Sendable (String) -> Void

    init(directory: URL = OllamaEngineDownloader.engineDir,
         temporaryDirectory: URL = FileManager.default.temporaryDirectory,
         downloadFile: @escaping Download = { try await OllamaEngineDownloader.downloadFile(from: $0, to: $1, progress: $2) },
         runCommand: @escaping Run = { try OllamaEngineDownloader.run($0, $1) },
         notify: @escaping @Sendable (String) -> Void = { SystemUserNotifier.post($0) }) {
    ...
    func download(progress: @escaping @Sendable (Double) -> Void) async throws {
        ...
        if fm.isExecutableFile(atPath: directory.appendingPathComponent("ollama").path) { return }

        let tmp = temporaryDirectory.appendingPathComponent("glosso-engine-\(UUID().uuidString)", isDirectory: true)
        try fm.createDirectory(at: tmp, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: tmp) }

        let zip = tmp.appendingPathComponent("Ollama-darwin.zip")
        try await downloadFile(Self.downloadURL, zip, progress)
        ...
        try runCommand("/usr/bin/ditto", ["-x", "-k", zip.path, tmp.path])
        ...
        do {
            ...
            notify(loc("Pobrany silnik nie przeszedł weryfikacji podpisu — instalacja przerwana.",
        ...
        try fm.moveItem(at: resources, to: directory)
    }
