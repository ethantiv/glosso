...
If you'd rather not download a 7–20 GB model, the wizard also offers **Google AI**: the same Gemma models plus Gemini Flash Lite, served by the Gemini API, free, with only an API key to paste. In that mode the selected text is sent to Google. Local stays the default.
...
Picking **Google AI** reveals a field for the API key ([free from Google AI Studio](https://aistudio.google.com/apikey)), a choice of model and today's request count. The key is stored in the Keychain. Glosso paces itself to each model's own free-tier limits — Gemma (the default) gets 30 requests and 16k input tokens per minute and 14 400 per day, Gemini Flash Lite 15 requests and 250k tokens per minute and 500 per day — waiting rather than getting rejected, with a separate daily counter per model, and it falls back to the local model when the daily quota runs out.
